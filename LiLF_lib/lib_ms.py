...
class AllMSs(object):
    ...
    def addcol(self, newcol, fromcol, usedysco=None, log='$nameMS_addcol.log', overwrite=True):
        """
        ...
        if check != 0:
            sm = '' # storagemanager
            if usedysco is None: # if col is dysco compressed in first MS, assume it is for all MSs
    ...
    def run_Blsmooth(self, incol='DATA', outcol='SMOOTHED_DATA', ionf='auto',  notime=False, nofreq=False, logstr='smooth'):
        ...
        self.run(f'/home/local/work/j.boxelaar/scripts/LiLF/scripts/BLsmooth.py -c {chunks} -n {ncpu} -f {ionf} -r -i {incol} -o {outcol} {extra_flags} $pathMS',
