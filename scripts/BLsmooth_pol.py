...
def smooth_baseline(in_bl, data, weights, std_t, std_f, outQueue=None):
    ...
    if options.onlyamp: # smooth only amplitudes
        ...
        if options.onlydiag:
            if not options.notime:
                dataAMP[:,:,0:4:3] = gfilter(dataAMP[:,:,0:4:3], std_t, axis=0, truncate=3)
            if not options.nofreq:
                dataAMP[:,:,0:4:3] = gfilter(dataAMP[:,:,0:4:3], std_f, axis=1, truncate=3)
        else:
            if not options.notime:
        ...
        if options.onlydiag:
            if not options.notime:
                dataR[:,:,0:4:3] = gfilter(dataR[:,:,0:4:3], std_t, axis=0, truncate=3)
                dataI[:,:,0:4:3] = gfilter(dataI[:,:,0:4:3], std_t, axis=0, truncate=3)
            if not options.nofreq:
                dataR[:,:,0:4:3] = gfilter(dataR[:,:,0:4:3], std_f, axis=1, truncate=3)
                dataI[:,:,0:4:3] = gfilter(dataI[:,:,0:4:3], std_f, axis=1, truncate=3)
        else:
            if not options.notime:
    ...
    if options.onlydiag:    
        if not options.notime:
            weights[:,:,0:4:3] = gfilter(weights[:,:,0:4:3], std_t, axis=0, truncate=3)
        if not options.nofreq:
            weights[:,:,0:4:3] = gfilter(weights[:,:,0:4:3], std_f, axis=1, truncate=3)
    else:
        if not options.notime:
...
opt.add_option('-d', '--onlydiag', help='Smooth only diagonal polarisations [default: smooth all pols]', action="store_true", default=False)
