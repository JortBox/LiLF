    ...
    for timestamp in set([ os.path.basename(ms).split('_')[1][1:] for ms in MSs_list ]):
        ...
        if os.path.exists(MS_concat_bkp): 
            ...
            if stats == "core":
                ...
                SCHEDULE.add(
                    f'DP3 {parset_dir}/DP3-avg.parset msin=\"{str(mss_toconcat)}\" msin.baseline="*&" msout={MS_concat}',
                    log=MS_concat+'_avg_'+stats+'.log', 
                    commandType='DP3'
                )
                SCHEDULE.run(check=True, maxThreads=1)                
    ...
    if stats == "all":
        with WALKER.if_todo('correctCS_' + stats):
            ...
            if len(fulljones_solution) != 0:
                Logger.info(f"Correction Ga of {fulljones_solution[-1]}")
                # correcting CORRECTED_DATA -> CORRECTED_DATA
                MSs.run(
                    f'DP3 {parset_dir}/DP3-cor.parset msin=$pathMS msin.datacolumn={data_column} \
...
class Selfcal(object):
    ...
    def solve_gain(self, mode:str) -> None:
        ...
        Logger.debug(f'Datacolumn: {self.data_column}')
        if mode == 'fast':
    ...
    def clean(self, imagename: str) -> None:
        ...
        kwargs1.update({"size": 2000}) # type: ignore
        kwargs2.update({"size": 2000}) # type: ignore
        kwargs1.update({"scale": "2.5arcsec"}) # type: ignore
        kwargs2.update({"scale": "2.5arcsec"}) # type: ignore
    ...
    for stations in calibration_modes:
        ...
        for cycle in calibration:
            ...
            with WALKER.if_todo(f"image-{stations}-c{cycle}" ):
                #imagename = 'img/img-' + stations + '-%02i' % cycle
