...
def make_beam_region(MSs: MeasurementSets):
...
def correct_from_callibrator(MSs: MeasurementSets, timestamp: str):
...
def setup(stats:str) -> None:
    ...
    for timestamp in set([ os.path.basename(ms).split('_')[1][1:] for ms in MSs_list ]):
        ...
        MS_concat = f'{TARGET}_t{timestamp}_concat_{stats}.MS'
        MS_concat_bkp = f'{TARGET}_t{timestamp}_concat_{stats}.MS-bkp'
        ...
        if os.path.exists(MS_concat_bkp): 
            ...
            MSs = MeasurementSets([MS_concat], SCHEDULE)
...
def phaseup(MSs:MeasurementSets, stats:str) -> MeasurementSets:
    data_column = "DATA"
    if stats == "all":
        with WALKER.if_todo('correctCS_' + stats):
            # Correct CS
            Logger.info('Correcting CS...')
            
            if os.path.exists("cal-tec1-c1.h5"):
                Logger.info("correction TEC1...")
                MSs.run(
                    f'DP3 {parset_dir}/DP3-cor.parset msin=$pathMS msin.datacolumn={data_column}\
                        cor.parmdb=cal-tec1-c1.h5 cor.correction=tec000',
                    log='$nameMS_corTEC-c1.log', 
                    commandType='DP3'
                )
                data_column = "CORRECTED_DATA"
            
            for i, solution in enumerate(sorted(glob.glob("cal-Gp*core.h5"))):
                Logger.info("correction Gp of "+str(solution))
                # correcting CORRECTED_DATA -> CORRECTED_DATA
                MSs.run(
                    f'DP3 {parset_dir}/DP3-cor.parset msin=$pathMS msin.datacolumn={data_column} \
                        cor.parmdb={solution} cor.correction=phase000',
                    log='$nameMS_corPH-core-'+str(i+1)+'.log', 
                    commandType='DP3'
                )
                data_column = "CORRECTED_DATA"
                
                if os.path.exists(f"cal-Ga-c{i+1}-core.h5"):
                    Logger.info(f"Correction Ga of cal-Ga-c{i+1}-core.h5")
                    # correcting CORRECTED_DATA -> CORRECTED_DATA
                    MSs.run(
                        f'DP3 {parset_dir}/DP3-cor.parset msin=$pathMS msin.datacolumn={data_column} \
                            cor.parmdb=cal-Ga-c{i+1}-core.h5 cor.correction=fulljones \
                            cor.soltab=[amplitude000,phase000]',
                        log='$nameMS_corAMPPHslow-core-'+str(i+1)+'.log', 
                        commandType='DP3'
                    )
        ...
        with WALKER.if_todo('phaseupCS_' + stats):
            ...
            MSs.run(
                f"DP3 {parset_dir}/DP3-phaseup.parset msin=$pathMS \
                    msin.datacolumn={data_column} msout=$pathMS-phaseup \
    ...
    if stats == "def":
        with WALKER.if_todo('phaseupCS_' + stats):
            ...
            Logger.info('Phasing up Core Stations...')
            lib_util.check_rm(f'*{stats}.MS-phaseup')
            command = f"DP3 {parset_dir}/DP3-phaseup.parset msin=$pathMS \
                    msin.datacolumn=DATA \
                    msout=$pathMS-phaseup " + "stationadd.stations={CSsuperLBA:'CS00[2-7]*'} " + "filter.baseline=^CS00[2-7]*&& "
            MSs.run(
                command, 
                log=f'$nameMS_phaseup.log', 
                commandType="DP3"
...
def predict(MSs: MeasurementSets, doBLsmooth:bool = True) -> None:
    ...
    MSs.run(
        f'DP3 {parset_dir}/DP3-predict.parset msin=$pathMS pre.sourcedb={sourcedb}', 
        log='$nameMS_pre.log', 
        commandType='DP3'
    )
...
def remove_stations(MSs: MeasurementSets, suffix: str = 'core') -> MeasurementSets:
    ...
    def solve_gain(self, mode:str) -> None:
        ...
        if mode == 'fast':
            ...
            self.mss.run(
                ...
                log=f'$nameMS_corPH-c{self.cycle:02d}.log', 
                commandType='DP3'
            )
...
def main():
    with WALKER.if_todo("clean_core"):
    ...
    for stations in calibration_modes:
        ...
        with WALKER.if_todo('setup_' + stations):
            #set up corected data
            setup(stats=stations)
        

        MSs = MeasurementSets(glob.glob(f'*concat_{stations}.MS'), SCHEDULE, check_flags=False)
        
        #subtract possible influence of A-Team sources
        # TODO: move to another place before averaging step
        #with WALKER.if_todo('demix_' + stations):
        #    demix(MSs)
        
        if stations != "core": 
            with WALKER.if_todo('phaseup_' + stations):
                # Buggy for DEF case
