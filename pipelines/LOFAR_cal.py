...
with w.if_todo('copy'):
    ...
    for MS in MSs.getListObj():
        MS.move(f'{copy_dir}{MS.nameMS}.MS', keepOrig=True, overwrite=False)
        
### DONE

lib_util.check_rm('concat.MS')
MSs = lib_ms.AllMSs( glob.glob(copy_dir+'/*MS'), s, check_flags = False )
...
with w.if_todo('cal_bp'):
    ...
    MSs.run(
        f'DP3 {parset_dir}/DP3-soldd.parset msin=$pathMS sol.h5parm=$pathMS/amp.h5 sol.mode=diagonal',
        log='$nameMS_solAMP.log', 
        commandType="DP3"
        )
...
### Flagging happens before this point
...
with w.if_todo('compressing_h5'):
    ...
    s.run()
    os.system('h5repack cal-pa.h5 cal-pa-compressed.h5; mv cal-pa-compressed.h5 cal-pa.h5') #gives error in losoto-final.log
...
for sol in solutions:
    file = sol.split("/")[-1]
    os.system(f"mv {sol} solutions/{file}")
