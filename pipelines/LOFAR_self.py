...
for c in range(2):
    ...
    if c == 0:
        with w.if_todo('solve_fr_c%02i' % c):
            ...
            MSs.run('/net/voorrijn/data2/boxelaar/scripts/LiLF_dev/scripts/mslin2circ.py -s -i $pathMS:CIRC_PHASEDIFF_DATA -o $pathMS:CIRC_PHASEDIFF_DATA',
    ...
    with w.if_todo('solve_tec1_c%02i' % c):
        ...
        MSs.run('/net/voorrijn/data2/boxelaar/scripts/LiLF_dev/scripts/BLsmooth.py -c 8 -n 8 -r -i CORRECTED_DATA -o SMOOTHED_DATA $pathMS', log='$nameMS_smooth-c'+str(c)+'.log', commandType='python')
        MSs.run('/net/voorrijn/data2/boxelaar/scripts/LiLF_dev/scripts/BLsmooth.py -c 8 -n 8 -r -i MODEL_DATA -o MODEL_DATA $pathMS', log='$nameMS_smooth-c'+str(c)+'.log', commandType='python')
    ...
    with w.if_todo('imaging_c%02i' % c):
        ...
        print(MSsClean.getBandwidth())
        print(MSsClean.getChout())
        print(4.e6)
        ...
        if c == 0:
            ...
            lib_util.run_wsclean(s, 'wsclean-c' + str(c) + '.log', MSsClean.getStrWsclean(), name=imagename,
                                 ...
                                 join_channels='', fit_spectral_pol=cc_fit_order, channels_out=MSsClean.getChout(4.e5), #CHANGED Orig:4.e6
        ...
        if c == 0:
            ...
            s.add('wsclean -predict -padding 1.8 -name img/wideM-'+str(c)+' -j '+str(s.max_processors)+' -channels-out '+str(MSs.getChout(4e5))+' '+MSs.getStrWsclean(), \
                   log='wscleanPRE-c'+str(c)+'.log', commandType='wsclean', processors='max')  #CHANGED Orig:4.e6
            s.run(check=True)
    ### DONE
    ...
    if c == 0:
        ...
        with w.if_todo('lowres_img_c%02i' % c):
            ...
            lib_util.run_wsclean(s, 'wscleanLR.log', MSs.getStrWsclean(), name=imagename_lr, do_predict=False,
                    ...
                    join_channels='', channels_out=MSs.getChout(2.e5))  #CHANGED Orig:2.e6

            s.add('wsclean -predict -padding 1.8 -name '+imagename_lr+' -j '+str(s.max_processors)+' -channels-out '+str(MSs.getChout(2e5))+' '+MSs.getStrWsclean(), \
                  log='wscleanLR-PRE-c'+str(c)+'.log', commandType='wsclean', processors='max')  #CHANGED Orig:2.e6
            s.run(check=True)
        ### DONE
        ...
        with w.if_todo('lowres_lsimg_c%02i' % c):
            ...
            lib_util.run_wsclean(s, 'wscleanLS.log', MSs.getStrWsclean(), name=imagename_ls, do_predict=False,
                                 ...
                                 auto_threshold=0.5, join_channels='', channels_out=MSs.getChout(4.e5)) #CHANGED Orig:4.e6
        ### DONE
...
with w.if_todo('imaging-pol'):
    ...
    lib_util.run_wsclean(s, 'wscleanP.log', MSs.getStrWsclean(), name=imagenameP, pol='QUV',
        ...
        join_channels='', channels_out=MSs.getChout(4.e5))  #CHANGED Orig:4.e6
